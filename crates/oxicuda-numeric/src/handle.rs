//! Handle and RNG primitives for `oxicuda-numeric`.
//!
//! Mirrors the pattern of `oxicuda-tn` / `oxicuda-cvx`.
//!
//! Provides:
...
//! - [`NumericHandle`] bundling SM version and a seeded RNG.
...
/// Top-level numerical analysis computation handle bundling SM version and a seeded RNG.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn handle_construction() {
        let h = NumericHandle::new(SmVersion::SM_90, 0);
    ...
    #[test]
    fn lcg_usize_bounds() {
        let mut r = LcgRng::new(321);
        for _ in 0..1000 {
            let n = r.next_usize(10);
            assert!(n < 10);
        }
    }
}
