//! Session handle for `oxicuda-gen`.
//!
//! `GenHandle` stores the compute device index, the GPU SM version,
...
/// Minimal PCG-style LCG random number generator for deterministic CPU-side
/// sampling in tests and buffer initialisation.
///
/// This uses a high-quality 64-bit LCG multiplier (Knuth MMIX):
...
impl LcgRng {
    ...
    ///
    /// # Panics
    /// Does not panic; if `n == 0` returns 0.
    #[inline]
...
/// Lightweight session descriptor for generative AI operations.
///
/// A `GenHandle` does **not** open a CUDA context; it merely records which
...
/// deterministic CPU-side sampling.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    #[test]
    fn gen_handle_default() {
        let h = GenHandle::default_handle();
        assert_eq!(h.device(), 0);
        assert_eq!(h.sm_version(), SmVersion(80));
    }

    #[test]
    fn gen_handle_custom() {
        let h = GenHandle::new(2, SmVersion(90), 12345);
    ...
    #[test]
    fn gen_handle_rng_mut() {
        let mut h = GenHandle::default_handle();
