...
mod e2e_tests {
    ...
    fn als_score_finite() {
        ...
        let mut model = Als::new(5, 8, 4, 0.01, &mut rng).unwrap();
        let interactions = vec![(0, 0, 1.0), (0, 2, 2.0), (1, 1, 1.0), (2, 3, 3.0)];
        model.fit(&interactions, 2).unwrap();
        let s = model.score(0, 0).unwrap();
    ...
    fn bpr_step_loss_finite() {
        ...
        let mut model = Bpr::new(5, 10, 4, 0.01, 0.001, &mut rng).unwrap();
    ...
    fn nmf_fit_no_error() {
        ...
        let model = Nmf::fit(&data, 3, 4, 3, 5, &mut rng).unwrap();
        let s = model.score(0, 0).unwrap();
    ...
    fn ncf_forward_in_0_1() {
        ...
        let model = Ncf::new(10, 20, 8, vec![16, 8], &mut rng).unwrap();
        let pred = model.forward(0, 0).unwrap();
        assert!(
    ...
    fn deepfm_forward_in_0_1() {
        ...
        let model = DeepFm::new(field_dims, 8, &[32, 16], &mut rng).unwrap();
        let field_ids = vec![0usize, 3, 2];
        let pred = model.forward(&field_ids).unwrap();
        assert!(
            (0.0..=1.0).contains(&pred),
    ...
    fn wide_deep_forward_in_0_1() {
        ...
        let model = WideDeep::new(16, &[32, 16], &mut rng).unwrap();
    ...
    fn sasrec_forward_finite() {
        ...
        let model = SasRec::new(50, 16, 2, 2, 20, &mut rng).unwrap();
        let item_ids = vec![0usize, 5, 12, 3];
        let logits = model.forward(&item_ids).unwrap();
    ...
    fn lightgcn_score_finite() {
        ...
        let mut model = LightGcn::new(5, 8, 8, 2, &mut rng).unwrap();
        let edges = vec![(0, 0), (0, 1), (1, 2), (2, 3), (3, 4)];
        model.propagate(&edges).unwrap();
    ...
    fn uniform_neg_not_in_positives() {
        ...
        let sampler = UniformNegSampler::new(100).unwrap();
        let mut positives = BTreeSet::new();
        positives.insert(0usize);
        positives.insert(1);
        positives.insert(2);
        for _ in 0..50 {
            let neg = sampler.sample(0, &positives, &mut rng).unwrap();
            assert!(
