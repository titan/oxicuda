//! Session handle for `oxicuda-audio`.
//!
//! `AudioHandle` stores the compute device index, the GPU SM version,
...
/// Lightweight session descriptor for audio model operations.
///
/// An `AudioHandle` does **not** open a CUDA context; it merely records which
...
mod tests {
    ...
    #[test]
    fn audio_handle_custom() {
        let h = AudioHandle::new(2, SmVersion(90), 12345);
    ...
    #[test]
    fn audio_handle_rng_mut() {
        let mut h = AudioHandle::default_handle();
