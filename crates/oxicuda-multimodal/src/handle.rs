//! Session handle for `oxicuda-multimodal`.
//!
//! `MultiModalHandle` stores the compute device index, the GPU SM version,
//! and a deterministic LCG random number generator for CPU-side operations
//! (weight initialisation, greedy decoding, random sampling).
...
/// Lightweight session descriptor for multi-modal operations.
#[derive(Debug, Clone)]
pub struct MultiModalHandle {
    sm: SmVersion,
    rng: LcgRng,
    device: u32,
}

impl MultiModalHandle {
    /// Create a new handle.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn multimodal_handle_default() {
        let h = MultiModalHandle::default_handle();
        assert_eq!(h.device(), 0);
        assert_eq!(h.sm_version(), SmVersion(80));
    }

    #[test]
    fn multimodal_handle_custom() {
        let h = MultiModalHandle::new(2, SmVersion(120), 99);
