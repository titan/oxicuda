//! Session handle for `oxicuda-continual`.
//!
//! `ContinualHandle` stores the compute device index, the GPU SM version,
//! and a deterministic LCG random number generator for CPU-side operations
//! (reservoir sampling, weight initialization, batch selection).
...
/// Lightweight session descriptor for continual learning operations.
#[derive(Debug, Clone)]
pub struct ContinualHandle {
    sm: SmVersion,
    rng: LcgRng,
    device: u32,
}

impl ContinualHandle {
    /// Create a new handle.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn continual_handle_default() {
        let h = ContinualHandle::default_handle();
        assert_eq!(h.device(), 0);
        assert_eq!(h.sm_version(), SmVersion(80));
    }

    #[test]
    fn continual_handle_custom() {
        let h = ContinualHandle::new(2, SmVersion(120), 99);
