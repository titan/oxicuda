//! RMSNorm — target: P6 (N=1024 rows × D=4096 hidden, memory-bandwidth bound).
//!
//! Measures throughput of [`oxicuda_dnn::norm::rms_norm`] for the canonical
//! LLM hidden-state shape (LLaMA / Gemma style):
//!
//! - input  `[N=1024, D=4096]`
//! - gamma  `[D=4096]`
...
fn bench_rmsnorm_d4096(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("dnn_p6_rmsnorm_d4096");
    group.throughput(Throughput::Elements(work));
    group.bench_function("oxicuda_f32_n1024_d4096", |b| {
        b.iter(|| {
            let _ = rms_norm(&handle, &input, &gamma, &mut output, EPS);
        });
    });
    group.finish();
}

criterion_group!(benches, bench_rmsnorm_d4096);
