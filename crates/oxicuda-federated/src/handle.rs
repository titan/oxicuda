//! Session handle for `oxicuda-federated`.
//!
//! `FedHandle` stores the compute device index, the GPU SM version,
//! and a deterministic LCG random number generator for CPU-side operations
//! (noise generation, client selection, test scaffolding).
...
impl LcgRng {
    ...
    /// Return a `u64` from combining two u32 draws.
    #[inline]
    ...
    /// Sample from Laplace(0, b) using inverse CDF method.
    /// Returns `−b * sign(u − 0.5) * ln(1 − 2*|u − 0.5|)`.
    pub fn next_laplace(&mut self, b: f32) -> f32 {
        let u = self.next_f32();
        let centered = u - 0.5;
        let abs_c = centered.abs();
        let safe = (1.0 - 2.0 * abs_c).max(1e-10);
        let sign = if centered >= 0.0 { 1.0_f32 } else { -1.0_f32 };
        -b * sign * safe.ln()
    }
}
...
/// Lightweight session descriptor for federated learning operations.
///
/// A `FedHandle` does **not** open a CUDA context; it merely records which
...
/// deterministic CPU-side sampling and noise generation.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    #[test]
    fn fed_handle_default() {
        let h = FedHandle::default_handle();
        assert_eq!(h.device(), 0);
        assert_eq!(h.sm_version(), SmVersion(80));
    }

    #[test]
    fn fed_handle_custom() {
        let h = FedHandle::new(2, SmVersion(90), 12345);
    ...
    #[test]
    fn fed_handle_rng_mut() {
        let mut h = FedHandle::default_handle();
        let v = h.rng_mut().next_f32();
        assert!((0.0..1.0).contains(&v));
    }

    #[test]
    fn lcg_rng_laplace_finite() {
        let mut rng = LcgRng::new(42);
        for _ in 0..100 {
            let v = rng.next_laplace(1.0);
