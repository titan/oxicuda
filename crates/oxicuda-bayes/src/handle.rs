...
//!
//! `BayesHandle` stores the compute device index, the GPU SM version,
...
/// Lightweight session descriptor for Bayesian model operations.
///
/// A `BayesHandle` does **not** open a CUDA context; it merely records which
...
mod tests {
    ...
    #[test]
    fn bayes_handle_default() {
        let h = BayesHandle::default_handle();
        assert_eq!(h.device(), 0);
        assert_eq!(h.sm_version(), SmVersion(80));
    }

    #[test]
    fn bayes_handle_custom() {
        let h = BayesHandle::new(2, SmVersion(90), 12345);
