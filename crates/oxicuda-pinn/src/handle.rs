//! Session handle for `oxicuda-pinn`.
//!
//! `PinnHandle` stores the compute device index, the GPU SM version,
//! and a deterministic LCG random number generator for CPU-side operations
//! (weight initialization, collocation sampling, Monte-Carlo trace estimation).
...
/// Lightweight session descriptor for PINN operations.
#[derive(Debug, Clone)]
pub struct PinnHandle {
    pub device: u32,
    pub sm: SmVersion,
    pub rng: LcgRng,
}
...
mod tests {
    ...
    #[test]
    fn pinn_handle_default() {
        let h = PinnHandle::default_handle();
        assert_eq!(h.device, 0);
        assert_eq!(h.sm_version(), SmVersion(80));
    }

    #[test]
    fn pinn_handle_custom() {
        let h = PinnHandle::new(2, SmVersion(120), 99);
