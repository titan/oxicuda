//! Session handle for `oxicuda-nas`.
//!
//! `NasHandle` stores the compute device index, the GPU SM version,
//! and a deterministic LCG random number generator for CPU-side operations
//! (parameter initialisation, architecture sampling, test scaffolding).
...
/// Lightweight session descriptor for NAS operations.
///
/// A `NasHandle` does **not** open a CUDA context; it merely records which
...
mod tests {
    ...
    #[test]
    fn nas_handle_default() {
        let h = NasHandle::default_handle();
        assert_eq!(h.device(), 0);
        assert_eq!(h.sm_version(), SmVersion(80));
    }

    #[test]
    fn nas_handle_custom() {
        let h = NasHandle::new(2, SmVersion(90), 12345);
    ...
    #[test]
    fn nas_handle_rng_mut() {
        let mut h = NasHandle::default_handle();
