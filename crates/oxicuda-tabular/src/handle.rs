//! Session handle and LCG RNG for `oxicuda-tabular`.
...
impl LcgRng {
    /// Create a new LCG RNG from the given seed (Knuth MMIX).
    #[must_use]
    ...
    /// Advance one step and return a `u32`.
    #[inline]
    ...
    /// Sample two independent N(0, 1) values via Box-Muller.
    ...
    /// Fill `buf` with N(0, std_dev) samples via Box-Muller.
...
impl TabularHandle {
    ...
    /// Create a handle with custom parameters.
    #[must_use]
...
mod tests {
    ...
    fn default_handle_fields() {
        let h = TabularHandle::default_handle();
