...
impl SmVersion {
    /// Return the raw SM version integer.
    #[must_use]
    #[inline]
    ...
    /// Map SM version to the corresponding PTX ISA version string.
    #[must_use]
    ...
    /// Return the PTX target string, e.g. `"sm_80"`.
    #[must_use]
...
impl LcgRng {
    ...
    /// Draw the next `u32` pseudo-random value.
    #[inline]
    ...
    /// Draw the next `u64` pseudo-random value.
    #[inline]
    ...
    /// Draw a standard-normal `f32` using Box-Muller transform.
    ...
    /// Draw a pair of independent standard-normal `f32` values.
...
/// Lightweight session descriptor for distillation operations.
#[derive(Debug, Clone)]
pub struct DistillHandle {
    /// The target SM version for PTX code generation.
    pub sm: SmVersion,
    /// Deterministic RNG for weight initialization and sampling.
    pub rng: LcgRng,
}

impl DistillHandle {
    /// Construct a new handle for the given SM version and seed.
    #[must_use]
    ...
    /// Return a mutable reference to the RNG.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn distill_handle_new() {
        let h = DistillHandle::new(80, 42);
