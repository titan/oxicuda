...
mod tests {
    ...
    fn skd_entropy_uniform() {
        ...
        let h = Skd::entropy(&logits).expect("entropy should succeed");
    ...
    fn skd_adaptive_alpha_entropy_high_gives_high_weight() {
        ...
        let alpha = Skd::adaptive_alpha(&logits, &mode).expect("adaptive_alpha should succeed");
        assert!(
            alpha > 0.7,
    ...
    fn skd_adaptive_alpha_confidence_mode() {
        ...
        let alpha_peaked =
            Skd::adaptive_alpha(&logits_peaked, &mode).expect("adaptive_alpha should succeed");
        assert!(
            alpha_peaked < 0.01,
    ...
    fn skd_loss_single_sample_finite() {
        let cfg = SkdConfig::new(4.0).expect("new should succeed");
        let s = vec![1.0_f32, 2.0, 3.0];
        let t = vec![1.1_f32, 2.1, 2.9];
        let loss = Skd::skd_loss(&s, &t, 2, &cfg).expect("skd_loss should succeed");
        assert!(
    ...
    fn skd_loss_teacher_equals_student() {
        ...
        let loss = Skd::skd_loss(&logits, &logits, 1, &cfg).expect("skd_loss should succeed");
    ...
    fn skd_loss_batch_shape() {
        ...
        let l1 = Skd::skd_loss(&s1, &t1, 2, &cfg).expect("skd_loss should succeed");
        let l2 = Skd::skd_loss(&s2, &t2, 2, &cfg).expect("skd_loss should succeed");
    ...
    fn skd_loss_batch_empty_error() {
        let cfg = SkdConfig::new(2.0).expect("new should succeed");
    ...
    fn skd_multi_teacher_shape() {
        ...
        let loss = Skd::multi_teacher_loss(&s, &[t1, t2], 2, &cfg, 2.0)
            .expect("multi_teacher_loss should succeed");
        assert!(
            loss.is_finite() && loss >= 0.0,
    ...
    fn skd_multi_teacher_uniform_teachers() {
        ...
        let loss_single = Skd::skd_loss(&s, &t, 2, &cfg).expect("skd_loss should succeed");
        let loss_multi = Skd::multi_teacher_loss(&s, &[t.clone(), t.clone()], 2, &cfg, 1.0)
            .expect("value should be present");
    ...
    fn skd_cosine_similarity_same_vector() {
        ...
        let sim = Skd::cosine_similarity(&v, &v).expect("cosine_similarity should succeed");
        assert!(
            (sim - 1.0).abs() < 1e-5,
    ...
    fn skd_cosine_similarity_orthogonal() {
        ...
        let sim = Skd::cosine_similarity(&a, &b).expect("cosine_similarity should succeed");
