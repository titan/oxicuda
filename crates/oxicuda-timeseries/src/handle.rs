//! Session handle for `oxicuda-timeseries`.
//!
//! `TsHandle` stores the compute device index, the GPU SM version,
...
///
/// Uses the Knuth MMIX 64-bit LCG:
...
impl LcgRng {
    ...
    /// Return a `usize` uniformly drawn from `[0, n)`. Returns 0 if `n == 0`.
    #[inline]
...
/// Lightweight session descriptor for time-series model operations.
///
/// Carries device ordinal, SM version for PTX JIT, and a seeded LCG RNG
/// for deterministic parameter initialisation.
#[derive(Debug, Clone)]
pub struct TsHandle {
    sm: SmVersion,
    rng: LcgRng,
...
impl TsHandle {
    ...
    /// Convenience constructor: device 0, SM 8.0, seed 42.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn ts_handle_custom() {
        let h = TsHandle::new(2, SmVersion(90), 12345);
    ...
    #[test]
    fn ts_handle_rng_mut() {
        let mut h = TsHandle::default_handle();
