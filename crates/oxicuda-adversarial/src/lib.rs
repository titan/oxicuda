...
pub mod prelude {
    ...
    pub use crate::metrics::robust_accuracy::{certified_accuracy, robust_accuracy};
...
mod e2e_tests {
    ...
    fn e2e_fgsm_pushes_away_from_target() {
        ...
        let x = target.clone();
        let adv = fgsm_attack(&x, 0.1, 0.0, 1.0, quadratic_loss_grad(&target)).unwrap();
        ...
        let adv2 = fgsm_attack(&x2, 0.1, 0.0, 1.0, quadratic_loss_grad(&target)).unwrap();
    ...
    fn e2e_pgd_l_inf_respects_eps_ball() {
        let target = vec![0.5_f32; 4];
        let x = vec![0.6_f32; 4];
        let cfg = PgdConfig::new(0.05, 0.02, 5, false).unwrap();
    ...
    fn e2e_pgd_l2_respects_eps_ball() {
        ...
        let x = vec![0.6_f32; 8];
        let cfg = PgdConfig::new(0.5, 0.1, 5, false).unwrap();
        let mut handle = AdvHandle::default_handle();
    ...
    fn e2e_mim_with_momentum_decay_zero_matches_pgd() {
        ...
        let adv = mim_attack(&x, 0.0, 1.0, &mim_cfg, quadratic_loss_grad(&target)).unwrap();
        for (a, o) in adv.iter().zip(x.iter()) {
    ...
    fn e2e_trades_loss_collapses_to_ce_when_clean_equals_adv() {
        ...
        let cfg = TradesConfig::new(6.0).unwrap();
        let l = trades_loss(&logits, &logits, &labels, 2, 3, &cfg).unwrap();
    ...
    fn e2e_mart_loss_finite_under_perturbation() {
        ...
        let labels = vec![1_usize, 2];
        let cfg = MartConfig::new(5.0).unwrap();
        let l = mart_loss(&clean, &adv, &labels, 2, 3, &cfg).unwrap();
        assert!(l.is_finite() && l > 0.0);
    }
    ...
    fn e2e_randomized_smoothing_constant_classifier_returns_top_class() {
        let mut handle = AdvHandle::default_handle();
        let cfg = RsConfig::new(0.25, 1024, 0.001).unwrap();
        let x = vec![0.5_f32; 8];
        // Constant classifier returns class 7 always.
        let (cls, _r) = certified_radius(&x, &cfg, handle.rng_mut(), |_| Ok(7_usize)).unwrap();
        assert_eq!(cls, 7);
    }
    ...
    fn e2e_ibp_propagates_through_relu() {
        let bounds_in = vec![
            IntervalBound::new(-1.0, 1.0).unwrap(),
            IntervalBound::new(-2.0, 2.0).unwrap(),
        ];
        ...
        let out = ibp_propagate(&bounds_in, &w, &b, 2, 2).unwrap();
    ...
    fn e2e_lipschitz_certified_radius_formula() {
        ...
        let r = lipschitz_certified_radius(2.0, 1.0).unwrap();
    ...
    fn e2e_robust_accuracy_perfect_attack_zero() {
        ...
        let acc = robust_accuracy(&pred_all_wrong, &labels).unwrap();
        assert!(acc.abs() < 1e-6);
        let asr = attack_success_rate(&pred_all_wrong, &labels).unwrap();
    ...
    fn e2e_certified_accuracy_threshold() {
        ...
        let acc = certified_accuracy(&pred, &labels, &radii, 0.5).unwrap();
