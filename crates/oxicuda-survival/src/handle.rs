//! Handle and RNG primitives for `oxicuda-survival`.
//!
//! Mirrors the pattern of `oxicuda-tn` and `oxicuda-pde`.
//!
//! Provides:
...
//! - [`SurvivalHandle`] bundling SM version and a seeded RNG.
...
impl LcgRng {
    ...
    pub fn next_exponential(&mut self, lambda: f64) -> f64 {
        let u = self.next_f64().max(1.0e-300);
        -u.ln() / lambda.max(1.0e-300)
    }
}

/// Top-level survival computation handle bundling SM version and a seeded RNG.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn handle_construction() {
        let h = SurvivalHandle::new(SmVersion::SM_90, 0);
        assert_eq!(h.sm.value(), 90);
    }

    #[test]
    fn lcg_exponential_positive() {
        let mut r = LcgRng::new(123);
        for _ in 0..100 {
            let v = r.next_exponential(2.0);
            assert!(v >= 0.0);
        }
    }
}
